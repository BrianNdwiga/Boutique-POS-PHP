...
if(isset($_REQUEST['action']) && !empty($_REQUEST['action'])){ 
    if($_REQUEST['action'] == 'addToCart' && !empty($_REQUEST['id'])){ 
        ...
        $served_by = strip_tags($_POST['served_by']); 
        ...
        if(empty($errorMsg)){ 
            ...
            $insertCust = $db->query("INSERT INTO customers (first_name, last_name, email, phone, address, created) VALUES ('".$first_name."', '".$last_name."', '".$email."', '".$phone."', '".$address."', NOW())"); 
             
            if($insertCust){ 
                $custID = $db->insert_id; 
                 
                // Insert order info in the database 
                $insertOrder = $db->query("INSERT INTO orders (customer_id, grand_total, created, served_by, status) VALUES ($custID, '".$cart->total()."', NOW(),  '".$served_by."', 'Pending')"); 
